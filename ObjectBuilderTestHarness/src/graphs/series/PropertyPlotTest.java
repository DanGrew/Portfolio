...
public class PropertyPlotTest {
   ...
   @Test public void shouldConstructWithBasicSafeData() {
      ...
      Series< String, Number > series = plot.constructSeries( search, sampleHorizontal, null, null, null );
   ...
   @Test public void shouldConstructWithBasicMissingData() {
      ...
      Series< String, Number > series = plot.constructSeries( search, sampleHorizontal, null, "DEFAULT", -202 );
    ...
    * {@link PropertyPlot#constructSeries(Search, PropertyType, String, Number)} test.
    */
   @Test public void shouldOrderSafeData() {
      PropertyType sampleVertical = new PropertyTypeImpl( "name", ClassParameterTypes.NUMBER_PARAMETER_TYPE );
      PropertyType sampleHorizontal = new PropertyTypeImpl( "horizontal", ClassParameterTypes.STRING_PARAMETER_TYPE );
      SeriesExtractor plot = new PropertyPlot( sampleVertical );
      ...
      Series< String, Number > series = plot.constructSeries( search, sampleHorizontal, GraphSort.StringAlphabetical, null, null );
      Assert.assertEquals( "anything", series.getData().get( 1 ).getXValue() );
      Assert.assertEquals( 25.0, series.getData().get( 1 ).getYValue() );
      Assert.assertEquals( "another", series.getData().get( 0 ).getXValue() );
      Assert.assertEquals( 2.0, series.getData().get( 0 ).getYValue() );
      Assert.assertEquals( "else", series.getData().get( 3 ).getXValue() );
      Assert.assertEquals( 234.4355, series.getData().get( 3 ).getYValue() );
      Assert.assertEquals( "blank", series.getData().get( 2 ).getXValue() );
